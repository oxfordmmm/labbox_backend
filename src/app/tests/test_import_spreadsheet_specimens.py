...
from app.tests.import_spreadsheet_testing_data import specimen_data, specimen_data2, combined_specimen_data, bad_specimen_data

def assert_specimen_record_matches(specimen_record: Specimen, specimen_entry: Dict[str, Any]):
...
@pytest.mark.asyncio
async def test_import_specimens_update_insert(
    ...
    result = await db_session.execute(select(Specimen).order_by(asc(Specimen.accession)))
    ...
    for specimen_record, specimen_entry in zip(specimen_records, combined_specimen_data):
        assert_specimen_record_matches(specimen_record, specimen_entry) 
    ...
    assert (
        logger_mock.mock_calls[0][1][0] == "Specimens Sheet Row 2: Owner blah owner, blah site does not exist"
    )
    assert logger_mock.mock_calls[1][0] == "info"
    assert (
        logger_mock.mock_calls[1][1][0] == "Specimens Sheet Row 2: Specimen adfs1, 2024-01-01, sponge bob does not exist"
    )
    assert logger_mock.mock_calls[2][0] == "info"
    assert (
        logger_mock.mock_calls[2][1][0] == "Specimens Sheet Row 3: Specimen adfs2, 2024-01-01, square pants does not exist"
    )

    # Check the log messages from the second import
    assert logger_mock.mock_calls[3][0] == "info"
    assert (
        logger_mock.mock_calls[3][1][0] == "Specimens Sheet Row 2: Specimen adfs2, 2024-01-01, square pants already exists"
    )
    assert logger_mock.mock_calls[4][0] == "info"
    assert (
        logger_mock.mock_calls[4][1][0] == "Specimens Sheet Row 3: Specimen adfs3, 2024-03-01, krusty krab does not exist"
    )
...
async def test_import_bad_specimen(
    ...
    # check the record count
    assert len(specimen_records) == 0, f"Expected 0 records, but found {len(specimen_records)}"
    ...
    assert logger_mock.mock_calls[0][1][0] == "Specimens Sheet Row 2 ('accession',) : String should have at most 20 characters"
    
    assert logger_mock.mock_calls[1][0] == "error"
    assert logger_mock.mock_calls[1][1][0] == "Specimens Sheet Row 2 ('collection_date',) : Input should be a valid date or datetime, month value is outside expected range of 1-12"
    
    assert logger_mock.mock_calls[2][0] == "error"
    assert logger_mock.mock_calls[2][1][0] == "Specimens Sheet Row 2 ('country_sample_taken_code',) : String should have at least 3 characters"
